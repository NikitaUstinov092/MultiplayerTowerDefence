...
{
    public sealed class InstantHitscanWeaponView : NetworkBehaviour
    {
        [SerializeField]
        ...
        [SerializeField]
        private InstantHitscanVfx[] _hitscanVfxArray;
        ...
        private void OnHitscan(int index, HitscanEventData hitscanEvent)
        {
            Vector3 endPosition = hitscanEvent.endPosition;
            Quaternion vfxRotation = Quaternion.LookRotation(-hitscanEvent.Direction);

            // Рисуется шлейф
            InstantHitscanVfx hitscanVfx = _hitscanVfxArray[index % _hitscanVfxArray.Length];
            hitscanVfx.Play(hitscanEvent.startPosition, endPosition);
